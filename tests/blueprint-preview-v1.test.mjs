...
test('Pro release promotes Preview lineage into the Amy-fx-pro main channel', async () => {
  ...
  assert.match(workflow, /AMYFX_VERSION_NAME:\s*"2\.0\.0-pro\.318"/);
  assert.match(workflow, /AMYFX_VERSION_CODE:\s*"950318"/);
  assert.match(workflow, /amyfx-pro-2\.0\.0-pro\.318/);
