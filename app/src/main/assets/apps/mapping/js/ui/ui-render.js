...
function scalperShadowPlaceholder() {
  return `<section id="amy-scalper-entry-watch" class="card scalper-watch scalper-watch--wait" data-scalper-mode="shadow" data-dom-persistent="true" data-stability-key="scalper-shadow">
    ...
    <div class="scalper-watch__notice">Mode simulasi — tanpa eksekusi broker otomatis.</div>
    <p class="scalper-watch__instruction">Memindai IFVG searah H1 dan FVG Buy High Quality dari candle tertutup.</p>
    <div class="scalper-watch__foot"><span>Engine MENUNGGU DATA</span><span>Maksimal 2 rekomendasi setup</span></div>
  </section>`;
}
...
export function setupCard(s, se, i = 0, mode = 'ACTIVE') {
  ...
  if (s?.components) {
    let c = s.components;
    comp = `<div class="num-grid" style="margin-top:10px;border-top:1px solid rgba(212,175,55,.16);padding-top:10px">
  ...
  let chk = '';
  if (s?.scoreChecklist) {
    let list = s.scoreChecklist.map(x => `<div style="font-size:12px;margin:2px 0"><span style="color:${x.passed ? '#00d97e' : '#ff4c4c'}">${x.passed ? '✓' : '×'}</span> ${x.name} <span class="muted">+${x.score}</span></div>`).join('');
    chk = `<div style="margin-top:10px;border-top:1px solid rgba(212,175,55,.16);padding-top:10px"><b>Checklist Score: ${s.score || 0}/100 — Grade ${s.grade || ''}</b><br>${list}</div>`;
  }
  let sess = '';
  if (s?.sessionContext) {
  ...
  let cfHtml = '';
  if (s?.conflictCheck) {
    let cf = s.conflictCheck, badge = cf.conflictLevel === 'NONE' ? '#00d97e' : cf.conflictLevel === 'FATAL' || cf.conflictLevel === 'HIGH' ? '#ff4c4c' : '#f5b942', cNotes = cf.conflicts.length ? cf.conflicts.map(x => x.note).join('<br>') : 'Komponen utama selaras.';
    cfHtml = `<div style="margin-top:10px;border-top:1px solid rgba(212,175,55,.16);padding-top:10px;font-size:12px"><b>Conflict: <span style="color:${badge}">${cf.conflictLevel}</span> — ${cf.recommendation}</b><br><span class="muted">${cNotes}</span></div>`;
  }
...
export function historyView(){return`<section class="card"><h1>Event Logs</h1><button class="action" onclick="window.downloadLogs()">Unduh TXT</button>${state.logs.map(x=>`<div class="log">${x}</div>`).join('')||'<p class="muted">Belum ada event.</p>'}</section>`}
